...
app.route('/api/products')
    ...
    .post(function(req, res){
        var product = Product.build(_.pick(req.body, ['name', 'price', 'description']));
...
app.route('/api/products/:product_id')
    ...
    .put(function(req, res){
        Product.findById(req.params.product_id).then(function(product){
            product.update(_.pick(req.body, ['name', 'price', 'description'])).then(function(product){
                res.json(product);
            });
        });
    })
    .delete(function(req, res){
        Product.findById(req.params.product_id).then(function(product){
            product.destroy().then(function(product){
                InvoiceItem.findAll().then(function(InvoiceProducts){
                    InvoiceProducts.forEach(function(Item){
                        if(req.params.product_id == Item._previousDataValues.product_id){
                            InvoiceItem.findById(Item._previousDataValues.id).then(function(invoice_item){
